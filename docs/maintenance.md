...
All static UI content and option definitions (`scenarios`, `perspectives`, `techCards`, `languages`, `presetModes`, `faqData`, `printingGuideData`) are centralized in `src/js/core/content.js`.
...
2. Each entry defines:
   ...
   - `perspective`, `techCards`, `scenario`: The exact state keys to preconfigure when selected.
3. For custom options (`Use your own Scenario` or `Own Technologies`), attach `isCustom: true` to the option definition inside `scenarios` or `techCards`.
...
*   `{ "techCards.isCustom": true }`: Matches whenever the selected tech cards object has `isCustom: true`.
