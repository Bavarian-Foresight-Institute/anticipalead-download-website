...
*   **Mode:** The package selection workflow mode (`preset1`, `preset2`, `preset3`, or `custom`). Selecting a preset mode (`preset1` to `preset3`) preconfigures specific `perspective`, `techCards`, and `scenario` values while hiding those configuration grids from the UI. Selecting `custom` reveals granular option selectors.
...
*   **Tech Cards (`techCards`):** Depends on `[language, techCards]`.
*   **Quickstart Guide (`quickstartGuide`):** Depends on `[language]`.
*   **Custom Material Guide (`customGuide`):** Depends on `[language]`. Included dynamically via `conditionsAny: [{ "scenario.isCustom": true }, { "techCards.isCustom": true }]` whenever a custom scenario (`Use your own Scenario`) or custom technology cards (`Own Technologies`) are selected.

The application avoids hardcoded `if/else` checks for individual scenario or tech cards IDs. Instead, it uses declarative template interpolation (`{language}`, `{scenario}`, `{perspective}`, `{techCards}`) and dot-notation metadata lookups (`scenario.isCustom`, `techCards.isCustom`) driven entirely by `config.json` and `content.js`.
