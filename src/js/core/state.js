...
import { scenarios, perspectives, techCards, languages, presetModes } from './content.js';
 ...
 * @param {string} perspective - The chosen perspective ('with_roles' or 'no_roles').
 * @returns {void}
...
/**
 * Purpose: Update the selected tech cards in the state.
 * @param {string} tc - The ID of the chosen tech cards.
 * @returns {void}
 * Logic reason: Validates the tech cards before updating to ensure only supported values are set.
 */
export function setTechCards(tc) {
    if (techCards.some(t => t.id === tc)) {
        state.techCards = tc;
        if (state.mode !== 'custom') state.mode = 'custom';
        notifyListeners();
    } else {
        console.warn(`Invalid tech cards ID: ${tc}`);
    }
}
...
export function setMode(id) {
    ...
    if (mode) {
        ...
        if (!mode.isCustom) {
            ...
            if (mode.techCards) state.techCards = mode.techCards;
        }
        notifyListeners();
    } else {
