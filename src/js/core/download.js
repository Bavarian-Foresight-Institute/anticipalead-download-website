...
export function getDownloadFilename(currentState) {
    const { scenario, perspective, techCards, language } = currentState;
    const perspectiveTag = perspective === 'with_roles' ? 'With_Roles' : 'No_Roles';
    return `AnticipaLead_Scenario${scenario}_${perspectiveTag}_${techCards}_${language.toUpperCase()}.zip`;
}
