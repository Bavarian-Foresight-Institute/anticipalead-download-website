...
import { getState, setScenario, setPerspective, setTechCards, setLanguage, setMode, subscribe, resetState, initState } from './core/state.js';
import { renderFooterContent, renderPackageContentCard, renderHowItWorksStep, renderScenarioCard, renderPerspectiveCard, renderTechCardsCard, renderLanguageCard, renderModeCard, renderSummaryRow, renderStepper, NavButton, NavLink, PrimaryButton, SecondaryButton, DarkButton, SolidButton, OutlineButton, TextIconButton, TextIconLink, renderPrintingGuideCards, renderFAQCards } from './ui.js';
import { IconRoleCards, IconCanvases, IconTechCards, IconAudioGuide, IconNameTags, IconQuickstart, IconDownloadLg, IconVideoLg, IconArrowLeftSm, IconArrowRightSm, IconEditSm, IconArrowLeftMd, IconDownloadSm, IconVideoSm, IconSpinner, IconCheckmarkDark, IconPlusSm, IconMinusSm, IconPrinterSm, IconInfoSm, IconUsersSm, IconClockSm, IconBriefcaseSm, IconTranslateSm, IconModeratorSm } from './ui/icons.js';
import { footerContent, packageContents, howItWorksSteps, scenarios, perspectives, techCards, languages, presetModes, printingGuideData, faqData, videoGuideUrl } from './core/content.js';
...
function cacheDOM() {
    ...
    DOM.techCardsGrid = document.getElementById('tech-cards-grid');
    ...
    DOM.iconModeratorContainer = document.getElementById('icon-moderator-container');
...
function renderUI(state) {
    ...
    // Render Tech Cards
    if (DOM.techCardsGrid) {
        if (DOM.techCardsGrid.children.length === 0) {
            DOM.techCardsGrid.innerHTML = techCards.map(t =>
                renderTechCardsCard({ ...t, isSelected: state.techCards === t.id })
            ).join('');
        } else {
            updateGridSelection(DOM.techCardsGrid, state.techCards, 'tech');
        }
    }
    ...
    if (btnPrintingGuide) {
        const perspective = state.perspective || 'no_roles';
...
function bindEvents() {
    ...
    // Tech Cards selection delegation
    if (DOM.techCardsGrid) {
        DOM.techCardsGrid.addEventListener('mousedown', (e) => {
    ...
    if (DOM.btnContinue) {
        DOM.btnContinue.addEventListener('mousedown', () => {
            ...
            const selectedTechCards = techCards.find(t => t.id === state.techCards);
            ...
            if (DOM.summaryTableBody) {
                if (!isCustomMode && selectedMode) {
                    ...
                    DOM.summaryTableBody.innerHTML = [
                        ...
                        renderSummaryRow('Tech Cards', selectedTechCards?.title || state.techCards)
                    ].join('');
                }
            }
                    ...
                    if (currentRequestId === estimationRequestId) {
                        if (missingFiles.length > 0) {
                            ...
                            if (!isCustomMode && selectedMode) {
                                ...
                                DOM.downloadInfo.innerHTML = `${filesCount} files - ${mb} MB <br> ${selectedLanguage.title} - ${selectedPerspective?.title || state.perspective} - Scenario ${selectedScenario?.id || state.scenario} - ${selectedTechCards?.title || state.techCards}`;
                            }
                        }
                    }
                });
...
function init() {
    ...
    if (DOM.iconModeratorContainer) DOM.iconModeratorContainer.innerHTML = IconModeratorSm;
    ...
    if (isDownloadPage) {
        ...
        const defaultTechCards = defaultMode.techCards || techCards[0].id;
        ...
        if (DOM.btnPrintingGuideContainer) {
            DOM.btnPrintingGuideContainer.innerHTML = OutlineButton({
                ...
                text: 'Printing guide',
                href: `./printing-guide.html#perspective=${getState().perspective || 'no_roles'}`,
                isFlexible: true,
                icon: IconPrinterSm
            });
        }
    ...
    if (isPrintingGuidePage) {
        ...
        let printingGuideVersion = 'with_roles'; // Default
        if (window.location.href.includes('perspective=with_roles')) {
            printingGuideVersion = 'with_roles';
        } else if (window.location.href.includes('perspective=no_roles')) {
            printingGuideVersion = 'no_roles';
        }
                ...
                if (DOM.printingGuideTabsContainer.children.length === 0) {
                    DOM.printingGuideTabsContainer.innerHTML = `
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-small w-full">
                            ${renderLanguageCard({ id: 'with_roles', title: 'With perspective changes', isSelected: printingGuideVersion === 'with_roles' })}
                            ${renderLanguageCard({ id: 'no_roles', title: 'Without perspective changes', isSelected: printingGuideVersion === 'no_roles' })}
                        </div>
                    `;
                } else {
